...
public interface ItemDiariaRepository extends JpaRepository<ItemDiaria, Long>{
	...
	@Query("From ItemDiaria d where d.unidade.id = ?1 AND d.funcionario.diaria.id = ?2")
	List<ItemDiaria> listaUnidade(Long id, Long idDiaria);
	
	@Query("From ItemDiaria d where d.unidade.coordenadoria.id = ?1 AND d.funcionario.diaria.id = ?2")
	List<ItemDiaria> findByUnidadeCoordenadoria_idAndDiaria_id(Long id, Long idDiaria);

	@Query("From ItemDiaria d where  d.funcionario.diaria.id = ?1")
