...
public class UnidadeRestController {
	...
	private UnidadeService unidadeService;
	 ...
	 public ResponseEntity<Unidade> salvar(@RequestBody Unidade unidade,UriComponentsBuilder ucBuilder){
		 ...
		 headers.setLocation(ucBuilder.path("/rest/unidade/salvar/{id}").buildAndExpand(unidade.getId()).toUri());
		 return new ResponseEntity<Unidade>(headers, HttpStatus.CREATED);
	 }

	 @RequestMapping(method = RequestMethod.PUT, value="/alterar")
	 public ResponseEntity<Unidade> alterarUsuario(@RequestBody Unidade unidade,UriComponentsBuilder ucBuilder){
		 unidadeService.salvarEditar(unidade);
		 HttpHeaders headers =new HttpHeaders();
		 headers.setLocation(ucBuilder.path("/rest/unidade/alterar/{id}").buildAndExpand(unidade.getId()).toUri());
		 return new ResponseEntity<Unidade>(headers, HttpStatus.CREATED);
	 }

	 
	 @RequestMapping(value = "/buscaPorId/{id}", method = RequestMethod.GET)
		public ResponseEntity<Unidade> buscarUsuarioPorId(@PathVariable Long id) {
